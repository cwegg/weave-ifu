...
class GuideStar:
    ...
    def __init__(self,ra,dec,pa,mode,nside=32,dither_pattern=None,max_guides=None):
        self.xml_template_url = 'http://casu.ast.cam.ac.uk/weave/data_model/misc/BlankXMLTemplate.xml'
    ...
    def select_target(self,annular=False):
        ...
        if (annular == False):
            ...
            _angles = [angles[ii] for ii in xrange(len(angles.keys()))]
            ...
            for index in xrange(len(self.guides_filter)):
            # for d in dist_sort:
                # i = i + 1
                sel = ''
                # index = indexOf(self.dist,d)
                if (index == 0) and (self.lifu):
                    sel = ' <-----'
        ...
        if self.lifu:
            if self.dither_pattern:
                dither_OK = seff.dither_check(guide_sel)
                if not dither_OK:
                    return None
            return guide_sel
